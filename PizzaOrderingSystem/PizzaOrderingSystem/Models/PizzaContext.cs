...
namespace PizzaOrderingSystem.Models
{
    public class PizzaContext:DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 1, name = "Pepperoni",amount = 30 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 2, name = "Mushroom",amount = 40 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 3, name = "Onions",amount = 50 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 4, name = "Bacon",amount = 70 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 5, name = "Extra Cheese", amount =80  });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 6, name = "Black Olives", amount = 89 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 7, name = "Green Peppers", amount = 100 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 8, name = "Sausage", amount = 150 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 9, name = "Spinach", amount = 80 });
            modelBuilder.Entity<Toppings>().HasData(new Toppings() { id = 10, name = "Pineapple", amount = 90 });
